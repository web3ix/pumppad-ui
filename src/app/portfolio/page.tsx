...
export default function PortfolioTokenPage() {
    const { publicKey } = useWallet();

    const { data, mutate, size, setSize, isLoading } = useSWRInfinite(
        (index) =>
            `${process.env.NEXT_PUBLIC_API}/bond/portfolio-tokens?user=${publicKey}&take=${DEFAULT_LIMIT}&skip=${index * DEFAULT_LIMIT}`,
        fetcher
    );
